...
const loadTxtToArray = (filePath) => {
  try {
    ...
    throw new Error(
      `Ошибка при загрузке данных ИНН, убедитесь что существует файл ${filePath} в корне проекта`
    );
  }
};
...
const getHTMLData = async (inn) => {
  return axios.get(
    `https://pd.rkn.gov.ru/operators-registry/operators-list/?act=search&name_full=&inn=${inn}&regn=`
  );
};
...
const startParser = async (list) => {
  ...
  for (const inn of list) {
    ...
    counter++;
    console.log(`Обработано ${counter} из ${list.length}`);
...
const formatDataToXlsxAndSave = () => {
  ...
  // Заголовки
  ws.cell(1, 1).string("ИНН");
  ws.cell(1, 2).string("Номер в реестре операторов ПД");
  ws.cell(1, 3).string("Наименование");
  ws.cell(1, 4).string("Тип оператора");
  ws.cell(1, 5).string("Основание включения в реестр");
  ws.cell(1, 6).string("Дата регистрации уведомления");
  ws.cell(1, 7).string("Дата начала обработки");

  let currentColumn = 2;
  for (const data of outputData) {
    ws.cell(currentColumn, 1).string(data.inn);
    ws.cell(currentColumn, 2).string(data.registerNumber);
    ws.cell(currentColumn, 3).string(data.operatorName);
    ws.cell(currentColumn, 4).string(data.operatorType);
    ws.cell(currentColumn, 5).string(data.inclusionInRegistry);
    ws.cell(currentColumn, 6).string(data.registrationDate);
    ws.cell(currentColumn, 7).string(data.startDateOfProcessing);
...
const bootstrap = async () => {
  ...
  console.log(
    `Локальные данные загружены (количество записей: ${innData.length})`
  );
